...
function AlertIcon({ type }: { type: Alert['type'] }) {
  if (type === 'obstacle' || type === 'error') return <AlertTriangle size={13} className="text-agc-red shrink-0" />
...
export function LandingPage() {
  const { telemetry, connected, status, alerts } = useVehicleStore()
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="space-y-2">
          <div className="text-xs text-agc-green uppercase tracking-widest font-semibold">IoT Control Center</div>
          <h1 className="text-4xl font-bold text-white leading-tight">
            AGC Autonomous<br className="sm:hidden" /> Agricultural Cart
          </h1>
          <p className="text-slate-400 text-sm max-w-md">
            Real-time monitoring, mission control, and route management for the autonomous yard utility vehicle.
          </p>
        </div>
        ...
        <div className="bg-agc-panel border border-agc-border rounded-xl p-5 space-y-3">
          ...
          {telemetry ? (
            <div className="space-y-2">
              ...
              <Link to="/dashboard" className="text-xs text-agc-blue hover:underline flex items-center gap-1">
